...
from .MobileConfig import MobileFaceNet

class ImgFeatures(object):
    ...
    def infer(self, batch_tensor):
        high, wide = batch_tensor.shape[2:]
        top = int(high / 2.1 * (0.8 - 0.33))
        bottom = int(high - (high / 2.1 * 0.3))
        size = bottom - top
        left = int(wide / 2 - size / 2)
        right = left + size
