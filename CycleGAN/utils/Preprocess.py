...
from .Seg_Face import Seg_Face
import numpy as np

class Preprocess:
    def __init__(self, device='cpu', detector='dlib'):
        self.detection = FaceDetection(device, detector)  # device = 'cpu' or 'cuda', detector = 'dlib' or 'sfd'
        self.segfcce = Seg_Face()

    def process(self, image):
        face_info = self.detection.align(image)
        if face_info is None:
            return None
        image_align, landmarks_align = face_info

        face = self.__crop(image_align, landmarks_align)
        mask = self.segfcce.get_mask(face)
